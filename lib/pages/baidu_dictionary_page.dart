...
class BaiduDictionaryPage extends HookConsumerWidget {
  const BaiduDictionaryPage({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      Future<void> loadData() async {
        final jsonString = await rootBundle.loadString('assets/baidu.json');
    ...
    if (isLoading.value) {
      return Scaffold(
        appBar: AppBar(title: const Text('百度词库')),
                      ...
                      return Padding(
                        ...
                        child: ListTile(
                          ...
                          onTap: () {
                            ...
                            final fileName = '${dictName.replaceAll(RegExp(r'[^\w一-龥]'), '_')}.bdict';
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          ...
          if (result != null && result.files.single.path != null) {
            ...
            if (!fileName.endsWith('.bdict')) {
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text("请选择 .bdict 格式的文件"),
                    behavior: SnackBarBehavior.floating,
                  ),
                );
              }
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => DictionaryPreviewPage(
                    loadData: () async {
                      final parser = ParserFactory.createParserByFormat(ImeFormat.baiduBdict);
                      var parseFile =await parser.parseFile(filePath);
                      return parseFile.entries;
                    },
                    dictionaryName: fileName.replaceAll('.bdict', ''),
                    category: 'baidu',
                    source: 'local',
                  ),
                ),
